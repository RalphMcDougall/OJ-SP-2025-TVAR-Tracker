...
for dataset_ind, dm in enumerate(testset.datasets):
    ...
    dim = data_generation_params.observation_dimensions
    model_order = 2
    polynomial_order = 1
    window = 10
    innovation_variance = 1E0
    jin_model = classic_models.JinTracker(dim, model_order, polynomial_order, window, innovation_variance, data_generation_params.measurement_params.noise_variance, ground_truth_position[:model_order,:][::-1])

    for ts in tqdm(range(model_order, data_generation_params.simulation_time), disable=False):
        measurements = cluttered_data[ts]

        meas_pred_mean, meas_pred_cov = jin_model.get_predicted_position_state()
        jin_pred_mean_history[dataset_ind, ts, :] = meas_pred_mean[:,0]
        jin_pred_cov_history[dataset_ind, ts, :, :] = meas_pred_cov
        meas_ind = np.argmin(classic_models.mahalanobis(measurements[:,:,None], meas_pred_mean, meas_pred_cov[None,:,:]))

        meas = measurements[meas_ind, :, None]
        obs_pred_mean, obs_pred_cov = jin_model.get_predicted_position_state()
        log_observation_predictions[dataset_ind, ts] = scipy.stats.multivariate_normal.logpdf(meas.flatten(), obs_pred_mean.flatten(), obs_pred_cov)

        jin_model.update(meas)

        m, P = jin_model.get_position_state()
        jin_mean_history[dataset_ind, ts, :] = m[:,0]
        jin_cov_history[dataset_ind, ts, :, :] = P

        """
        print(" >", meas)
        print("M", jin_mean_history[ts])
        print("P", jin_cov_history[ts])
        print("GT", ground_truth_position[ts])
        print("RMSE", classic_models.get_rmse_gauss(ground_truth_position[ts], jin_mean_history[ts], jin_cov_history[ts]) / np.sqrt(data_generation_params.measurement_params.noise_variance))
        print("")
        """
...
log_lik_results = np.zeros((3, len(testset.datasets)))
rmse_results = np.zeros((3, len(testset.datasets)))
predictive_rmse_results = np.zeros((3, len(testset.datasets)))
gt_lik_results = np.zeros((3, len(testset.datasets)))
...
for dataset_ind, dm in enumerate(testset.datasets):
    ...
    gt_lik_results[JIN_IND,dataset_ind] = jin_pred_lik 
