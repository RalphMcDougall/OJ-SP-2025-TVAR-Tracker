...
class InteractingMultipleModel:
    ...
    def update(self, measurement : np.ndarray):
        ...
        for j in range(self.num_models):
            ...
            if np.allclose(pred_meas_cov, pred_meas_cov.T) and np.all(np.linalg.eigvals(pred_meas_cov) > 0):
...
def kf_with_nn_association(model : KalmanFilter, cluttered_data : np.ndarray, params : settings.ClassicalStructuralTrackerParameters, verbose=True):
    ...
    for ts in tqdm(range(model.order, params.simulation_time), disable=not verbose):
        ...
        log_meas_probs = -0.5 * (mahalanobis(measurements[:,:,None], meas_pred_mean, meas_pred_cov[None,:,:]) ** 2)
        meas_probs = np.exp(log_meas_probs - np.logaddexp.reduce(log_meas_probs))
        meas_ind = np.random.choice(meas_probs.size, p=meas_probs)
...
def mahalanobis(data, mean, cov):
    diff = data - mean 
    squared_distance = foo_utils.transpose_last2(diff) @ foo_utils.inv(cov) @ diff
    if np.any(squared_distance) < 0:
        print(f"{colorama.Fore.RED}Note: Found negative distance with cov:\n{cov}")
        return np.inf
    return np.sqrt(squared_distance).flatten()
